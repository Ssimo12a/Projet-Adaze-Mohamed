...
window.options = {};
options.lineSize = 0.1;
options.fps = 30;
options.focalBlankSize = 1;
options.newLineIntervalMS = 1000;
options.lineGrowAmount = 0.003;
options.lineVelocityIncrease = 0.02;
options.lineDY = 0.3;
options.lineColor = "#4206f1"; // #FE18D3
options.horizonColor = "";
options.trackColor = `#270245`;
options.backgroundColor = `#000000`;

options.starSize = 0.25;
options.minMax = 8;
options.blurAmount = 0;
options.focalBlankSize = 8;
options.newStarsPerFrame = 1;
options.growAmount = 0.01;
options.velocityIncrease = 0.00025;
...
const stars = navigator.userAgent.match(/headless/i) ? starsCache : [];
setTimeout(() => {
console.log(stars);
}, 5000);
...
options.xStart = canvas.width / 2;
options.lineYStart = canvas.height / 1.25;
// stars.length = 0; // empty stars array
drawStuff();
}
function getLine() {
return {
  x: options.xStart || canvas.width / 2,
  y: options.lineYStart || canvas.height / 1.25,
  // not used for line
  dX: getRandomInt(-options.minMax, options.minMax) + getRandomInt(0, 9) / 10,
  dY: options.lineDY, //getRandomInt(-options.minMax, options.minMax) + getRandomInt(0, 9) / 10,
  size: options.lineSize,
  color: options.lineColor, //`rgb(${getRandomInt(200, 255)}, ${getRandomInt(200, 255)}, ${getRandomInt(200, 255)})`,
  type: "line"
};
}
function getStar() {
return {
  x: options.xStart || canvas.width / 2,
  y: options.yStart || canvas.height / 2,
  dX: getRandomInt(-options.minMax, options.minMax) + getRandomInt(0, 9) / 10,
  dY: getRandomInt(-options.minMax, options.minMax) + getRandomInt(0, 9) / 10,
  size: options.starSize,
...
resizeCanvas();
let lastFps = options.fps;
let handle = setInterval(mainLoop, 1000 / options.fps);

function mainLoop() {
if (options.fps !== lastFps) {
  clearInterval(handle);
  handle = setInterval(mainLoop, 1000 / options.fps);
}
...
// add new star
addStars(options.newStarsPerFrame);
addLine(options.newLineIntervalMS);
drawStuff();
}

function adjustControl(optionStr, delta) {
document.querySelector(`#options-${kabobCase(optionStr)}`).value =
  parseFloat(
    document.querySelector(`#options-${kabobCase(optionStr)}`).value,
    10
  ) + delta;
options[optionStr] = parseFloat(
  document.querySelector(`#options-${kabobCase(optionStr)}`).value,
  10
);
}
...
function drawStuff() {
if (KEYBOARD[LEFT]) {
  adjustControl('xStart', -5)
}
if (KEYBOARD[RIGHT]) {
  adjustControl('xStart', 5)
}
if (KEYBOARD[UP]) {
  adjustControl('velocityIncrease', .01)
}
if (KEYBOARD[DOWN]) {
 adjustControl('velocityIncrease', -.01)
}
...
stars.forEach((star) => {
  if (star.type === "star") {
    star.dX *= 1 + options.velocityIncrease;
    star.dY *= 1 + options.velocityIncrease;
    star.x += star.dX; // calculate star's new position
    star.y += star.dY;
    star.size +=
      ((Math.abs(star.dX) + Math.abs(star.dY)) / 2) * options.growAmount;
    for (let i = options.blurAmount + 1; i > 1; i--) {
...
ctx.fillStyle = options.trackColor;
ctx.fillRect(0, options.lineYStart, canvas.width, canvas.height);

ctx.strokeStyle = options.lineColor;
ctx.lineWidth = 2;
ctx.beginPath();
ctx.moveTo(options.xStart, options.lineYStart);
ctx.lineTo(options.xStart, canvas.height);
const startXOffset = canvas.width > 500 ? 100 : 25;
const endXOffset = canvas.width > 500 ? 200 : 200 / 4;
for (let i = 0; i <= 12; i++) {
  // right side
  ctx.moveTo(options.xStart + i * startXOffset, options.lineYStart);
  ctx.lineTo(options.xStart + i * endXOffset, canvas.height);
  // left side
  ctx.moveTo(options.xStart + i * -startXOffset, options.lineYStart);
  ctx.lineTo(options.xStart + i * -endXOffset, canvas.height);
}
...
stars.forEach((star) => {
  if (star.type === "line") {
    star.dX *= 1 + options.lineVelocityIncrease;
    star.dY *= 1 + options.lineVelocityIncrease;
    // star.x += star.dX; // calculate star's new position
    star.y += star.dY;
    star.size +=
      ((Math.abs(star.dX) + Math.abs(star.dY)) / 2) * options.lineGrowAmount;
...
// ctx.fillStyle = options.horizonColor;
// ctx.fillRect(
//   0,
//   options.lineYStart - options.focalBlankSize / 2,
//   canvas.width,
//   options.focalBlankSize
// );
}
...
const configDiv = document.querySelector(".options");
